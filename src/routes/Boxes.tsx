...
function BoxesList({ moveId }: { moveId: string }) {
  ...
  async function addBox() {
    ...
    // Try several helper names the store may expose
    ...
    // Fallback: take the last from the list if creation didn’t return
    if (!created) {
      const list: Box[] = (await call("listBoxes", moveId)) || [];
      created = list[list.length - 1];
    }

    // Ensure the new name is actually saved (some helpers default to “New Box”)
  ...
  async function deleteBox(id: string) {
    ...
    // Hard verify deletion (defensive for Dexie)
    const still = ((await call("listBoxes", moveId)) || []).some(
      (b: Box) => b.id === id
    );
    if (still) {
      try {
...
function BoxDetail({ moveId, boxId }: { moveId: string; boxId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ).sort((a: Item, b: Item) => (b.updatedAt || 0) - (a.updatedAt || 0));
  ...
  return (
    <div className="space-y-6">
      {/* Top-left Save & Return (replaces old Back + "Box") */}
