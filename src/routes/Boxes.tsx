  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {boxes.map(b => (
          <div key={b.id} className="card p-3 sm:p-4 flex flex-col">
            <div className="flex items-start justify-between gap-2">
              <InlineEditable value={b.name} onSave={v=>updateBox(b.id, { name: v })} className="text-lg font-semibold" />
              <span className="badge badge-sm border-neutral-300">Items: {stats[b.id] ?? 0}</span>
            </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              <button className="btn btn-ghost btn-sm" onClick={()=> nav(`/moves/${moveId}/boxes/${b.id}`)}>
                Add / View Items
              </button>
