    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {moves.length === 0 && (
            <p className="text-neutral-600 mt-2">Start by creating your first move. You can add boxes, items, images, and print labels with QR codes.</p>
          )}
        </div>
      ...
      {moves.length === 0 && (
        <div className="card p-6">
          <ol className="list-decimal pl-5 space-y-2">
            ...
            <li>Add boxes (room, status, images)</li>
            <li>Add items to each box (use Enter to add fast)</li>
          ...
          {moves.map(m => (
            <div key={m.id} className="card p-4 flex flex-col">
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button className="btn btn-ghost" onClick={()=>{ const name = prompt('Duplicate name', m.name + ' (copy)'); if(name){ makeMove(name); } }}>Duplicate</button>
